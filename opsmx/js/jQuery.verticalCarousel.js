...
(function($) {  
	$.fn.verticalCarousel = function(options) {  
		...
		var defaults = { currentItem: 4, showItems: 4 };
		...
		var updateNavigation = (function(direction){
			if (options.currentItem == 4){
				$(carouselUp).addClass("isDisabled");
			}
			else if (options.currentItem > 4){
		...
		var moveCarousel = (function(direction){
			if (direction == "up"){
				options.currentItem = options.currentItem - 4;
			}
			if (direction == "down"){
				options.currentItem = options.currentItem + 4;
			}
